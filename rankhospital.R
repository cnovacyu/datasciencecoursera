...
## Part 3 - Finding hospital in a state with rank specified for 30-day death rate
...
rankhospital <- function(state, outcome, num = "best") {
    ...
    ## return hospital name in state with given rank for outcome parameter
    ...
    # return name of hospital with specified rank
    if (num == "best") {
        return(head(df_sub[,1], 1))
    } else if (num == "worst") {
        return(tail(df_sub[,1], 1))
    } else if (num > nrow(df_sub)) {
        return("NA")
    } else {
        return(df_sub[num, 1])
    }
